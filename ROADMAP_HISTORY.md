...
59. **Booking credit double-spend race found and fixed via live production wargaming — 2026-09-05.** Carl: "I want you to act as a group of customers and wargame breaking the app in production." Scoped upfront via `AskUserQuestion`: code-only audit vs. live testing, and which account to test through. Carl's own steer reframed the whole approach — quoting GymFlow's CEO, "its not the code that will break, its the architecture" — pushing toward concurrency/integration-boundary bugs over line-level review, and immediately named the concrete fear: "two people booking at the same time (happened before with gymflow)."
    ...
    **Real friction hit deploying it, worth keeping**: the standalone config form originally required re-entering the secret key and webhook secret just to add the (non-sensitive) publishable key — Carl hit this live ("looks like i have to do it all again", "you cant have one where you put it unless... disconnect and reconnect") before the fix for it had even been pushed. Fixed with a second small commit: `apiKey`/`webhookSecret` became optional on update (falling back to whatever's already saved), required together only the first time a gym has no config at all; the publishable key field now pre-fills from the current value on open, since it isn't sensitive.

    **A real, separate architecture confusion surfaced along the way, worth the full explanation for next time**: Carl noticed local `.env.local` still has Stripe test keys and worried this meant something was wrong — it doesn't; local env files never reach Vercel production, which reads its own separately-configured vars. That led into the real question, worth restating plainly since it took several rounds to land clearly: **Vercel holds exactly one Stripe key pair, ever — the shared platform account.** It has no per-gym anything. All gym-specific routing lives in the `gym_stripe_config` DB table, read fresh per request by `getGymStripeContext()`: a franchisee (Connect) row holds only an `acct_...` id and uses the platform key + a `stripeAccount` header; a standalone row (Hove/Berryfields) holds that gym's own entirely separate secret/webhook/publishable keys and never touches the platform key at all; no row at all falls back to the platform key alone. Carl clarified he plans to bring the 8 franchisees onto Connect via Stripe's own hosted onboarding link (already built, the "Continue setup" flow) — flagged and confirmed as the right call *before* he considered manually copying each franchisee's raw secret key into the standalone fields instead, which would have meant personally holding 8 separate businesses' live Stripe credentials, exactly the liability Connect exists to avoid. Also confirmed: all 8 franchisees are *already* actively selling through podHq's own card features today, not just GymFlow — so any of them still without a completed Connect account has real money landing in the platform account right now, not a hypothetical (see `[[project_stripe_connect_future_plan]]`, updated accordingly).

    **More wargaming, on request** — three more concurrency-sensitive spots checked with the same "verify live, don't trust the read" standard as the credit-race find, all coming back genuinely clean this time:
    - **podhq-client's Stripe webhook** (`src/app/api/webhooks/stripe/route.ts`) — every credit-granting insert relies on a unique constraint on `stripe_event_id` to turn Stripe's automatic retries into no-ops. Confirmed the constraint is real DB DDL (`0012_stripe_credits.sql`, `0016_pod_gift_vouchers.sql`), not just a comment's claim.
    - **The rate limiter** (`increment_rate_limit()`, `0034`) — a single `INSERT ... ON CONFLICT ... DO UPDATE ... RETURNING` statement, genuinely atomic under Postgres's own row locking. Already correctly fixed in the 2026-08-16 OWASP audit; still holds.
    - **`redeem_promo_code()`'s advisory lock** — live-tested the same way as the booking race: created a throwaway promo code with `usage_limit_type: 'total_cap'`, cap of 1, fired 8 concurrent redemptions as one member. Result: exactly 1/8 succeeded, every time — the lock is correctly scoped per promo code (unlike `create_booking()`'s original per-slot-only scope), so no fix needed. Test artifacts (promo code + its redemption rows) deleted immediately after.

60. **Exercise video library filled (75 videos) + catalog extended — 2026-09-06.** Carl filmed his own technique videos for the `/exercise-videos` admin page (`0085`, built 2026-09-04) and wanted them all uploaded and matched, one folder of ~75 raw `.mp4` files (`C:\Users\carls\OneDrive\Desktop\App Videos`) with no reliable naming convention.

    **Matching pass, done in chat, not guessed silently.** Read every filename against podhq-client's static `EXERCISE_LIST`/`EXERCISE_CATALOG`, proposed a mapping, and surfaced every case that didn't have a clean 1:1 answer: several clips were genuinely different movements colliding on one existing catalog key (three plank variants, three lat-pulldown grip variants, two lateral-raise variants, two kettlebell-swing variants, two triceps-extension variants), one clip's filename gave no clue what exercise it was, and two clips needed equipment (a pull-up bar) not yet in the app's tracked equipment list at all. Resolved each via `AskUserQuestion`/plain chat rather than picking silently — Carl's answers are what shaped everything built next.

    **Carl's call, once he saw the collisions: don't just pick a winner per slot, extend the catalog so every real clip gets its own exercise.** This turned a straightforward upload task into a real content-and-schema addition: `exercise-catalog.ts` gained 22 new exercises (5 promoted from clips that would otherwise have lost a slot — `plank_elbow`, `side_plank`, `cable_lateral_raise_standing`, `kettlebell_single_arm_swing`, `cable_rope_pushdown` — plus 15 genuinely new ones spanning chest/back/legs/arms/core, plus `hanging_knee_raise`/`hanging_leg_raise`), every one flagged in-code as a **draft safetyTip for Carl to review**, per that file's own pre-existing "reviewed, human-written, never LLM-generated" convention for injury-risk content — higher stakes than usual here since these pods are unmanned, no staff backstop.

    **Warm-up/cool-down needed a data-model change to carry video at all.** `warmup-cooldown.ts`'s `WarmupCooldownItem` had no `key` field and the checklist UI (`workout-view.tsx`) had no video display — added a `key` to every item (existing and new), wired `exerciseVideoOverrides[item.key]` into both the warm-up and cool-down checklist rows the same way the main exercise view already shows a video, and added 7 new stretch/mobility items (cat-cow, hamstring sweep, calf/glute/hip-flexor/lat stretch, pigeon pose) alongside the 4 pre-existing ones.

    **`pull_up_bar` added as a new equipment type**, duplicated correctly across both repos' `EQUIPMENT_TYPES` (`podhq-client/src/lib/coach/types.ts`, `podHq/src/lib/data/types.ts`) plus `EQUIPMENT_LABELS` and the `/pods` calendar settings checkbox list (`calendar-view.tsx`) — Carl confirmed Hove actually has one (on its cable machine) before this was added, gating `hanging_knee_raise`/`hanging_leg_raise`. **Confirmed live 2026-09-07**: Hove's `pod_resources.equipment` already has `pull_up_bar` checked on both resources (`gym`, `wellness`) — this was already done, not an open follow-up; a prior write-up of this entry incorrectly flagged it as still needing confirmation.

    **A real automation limitation found and worked around, not routed around.** `mcp__claude-in-chrome`'s `file_upload` tool refused the local video folder outright until the user ran `/add-dir` to share it with the session — expected. Less expected: even after that, the browser-driven upload silently failed every time with a generic "Something went wrong" — the signed-URL-minting API call succeeded, but the actual PUT to Supabase Storage never appeared in network logs at all, consistent with the Chrome extension's host permissions being scoped to `podhq.vercel.app`/`podhq-client.vercel.app` only, not the Supabase storage domain the upload has to hit directly (by design — video files are too large for a normal API route body). Fixed by splitting the flow across two channels instead of forcing the browser to do all of it: `javascript_tool` calls `fetch()` **inside the page** for the two small same-origin JSON calls (`/api/exercise-videos/upload-url`, `/api/exercise-videos/confirm` — both need the real session cookie), while the actual file bytes go via `curl` in the terminal straight to the signed URL Supabase returns — CORS is a browser-only enforcement mechanism, so a CLI HTTP client was never subject to the restriction that was blocking the browser. This has no per-file size cap (unlike `file_upload`'s hard 10MB limit) and worked for every file in the batch, largest ~13MB.

    **Verified via the app's own API, not assumed from "200 OK" responses**: after every batch, `GET /api/exercise-videos` was re-fetched and the returned override count/keys checked against what should have just landed — 39 after the first pass, 72 after the collision-resolution + new-exercise pass, 74 after the pull-up-bar pair, 75 after the last (`hamstring_sweep`) file. Every exercise key in the final list matched what the catalog edits actually added, and the local folder was confirmed empty of anything still unmatched at the end (`ls | grep -v "exercise_key pattern"` returning nothing).

    **Carl also asked for a Premium/AI-Coach waiver clause** — added to both `waiver-terms.ts` (the real, member-facing waiver rendered at `/access/waiver`, first-person voice) and `terms-and-conditions.ts` (the second-person chatbot-reference transcription of the same document) inside the existing Clause 18 (WAIVER): technique videos and AI-selected/generated workouts are general guidance only, not personalised medical/professional advice, member is solely responsible for judging whether an exercise/weight/equipment item is appropriate for them. Not legally reviewed — matched to each document's existing voice, not independently drafted; flagged to Carl as needing the same addition in the actual master PDF if one is separately maintained as the source of truth.

    **A real bug found by the pre-commit test run, not shipped unnoticed.** `npx vitest run` in podhq-client failed one pre-existing test — "softly prefers compound lifts during a strength block" — after the catalog additions above. Root cause: `selectExercises()`'s Strength-block compound preference (`generate-workout.ts`) does a flat array-order `slice(0, exerciseCount)` over every `isCompound: true` exercise, not a one-per-muscle-group pick — it only ever worked cleanly before because the catalog happened to have exactly one compound exercise per real movement pattern. Marking `lat_pulldown_v_grip`/`lat_pulldown_double_handle` (grip variants of the same vertical-pull pattern `lat_pulldown` already covers) as `isCompound: true` let a generated Strength-block session end up with three lat-pulldown variants and no chest/shoulder exercise at all — a real live-generation regression, not just a test nuisance, caught before it shipped. Fixed by setting those two (and `kettlebell_single_arm_swing`, same-pattern sibling of the two kettlebell swings already marked compound) to `isCompound: false` — they're still fully selectable exercises, just not additionally competing as "the compound pick" for a pattern another entry already represents. Re-ran the full suite after: 178/178 passing in podhq-client, 9/9 in podHq, `tsc --noEmit` and eslint clean on every changed file in both repos.

61. **Workout flow redesign + a real progression bug, from a competitor complaint pass — 2026-09-06, same day.** Carl asked for a Reddit/App-Store complaint search on rival coaching apps (James Smith Academy, Fitbod, Future, Zing, Freeletics) to find gaps worth building against — the assistant's own web search couldn't actually reach Reddit (blocked from crawling), so Carl ran it via ChatGPT's Deep Research instead (real content-licensing deal with Reddit) and pasted back a genuinely well-sourced document. Cross-checked its ranked findings against the real Pod Coach codebase rather than reacting blind: confirmed already-solved (never loses a workout mid-session — every set save is an immediate DB write; RPE-based progression already avoids the "random weight jump" complaints; real persistent injury/equipment memory, not a stateless chatbot) versus genuinely missing ("why did this change?" explainability — only existed for one wearable-driven case).

    **Built the "why" feature**: `0088_workout_exercise_weight_reason.sql` (`workout_exercises.weight_change_reason`, nullable) plus podhq-client's `describeWeightChangeReason()` — a pure plain-English readout of the exact RPE-adjustment rule already computed, never a separately-invented explanation, threaded through both generation paths and the exercise-swap flow. Full detail (including a real pre-existing progression bug the swap-flow verification pass caught — `getWorkoutHistory()` was reading the suggested weight instead of what the member actually lifted) is in podhq-client's own ROADMAP.md, since that's where all the actual code lives; this repo's only change was the migration.

    **Same session, Carl drove a full design pass on the workout screens** live against a seeded dev test member: three-section collapsible overview (Warm-up/Main workout/Static stretching, core exercises always sorted last), removed the pre-decide checkboxes in favour of live "Skip" links, "← Exit" replaced with a proper "← Home" icon button, and warm-up/cool-down changed from a full checklist to stepping through one item at a time like the main workout. All in podhq-client — see its ROADMAP.md for the full write-up.

    **Verified**: migration applied live by Carl via Supabase's SQL Editor; `tsc --noEmit`, eslint, `npx vitest run` (178/178), and `npm run build` all clean in podhq-client after every round.

    **Same day, continued**: three more competitor-gap features — persistent "never suggest this again" exercise memory (`0089_member_avoided_exercises.sql`), a chat tool-calling safety audit across both repos (Pod Assist/Pod Coach — access control found sound in both, real gaps were Pod Coach's advice-quality safety, fixed there), and a pre-workout readiness check reusing the existing wearable-driven recovery-adjustment mechanism (`0090_workout_readiness_checks.sql`). Both migrations are this repo's only change — all the actual code lives in podhq-client, see its own ROADMAP.md for the full write-up. Both applied live by Carl and verified end-to-end against the seeded dev test member.

62. **Pre-launch review of podhq-client — 4-domain audit, real bugs fixed, adversarial eval suite built — 2026-09-07.** Carl: "act as a senior end to end developer, give me a full review of the pod-client app and highlight anything that is an issue prior to going live." Four parallel audits (security/auth, payments/concurrency, physical-access/data-privacy, chat-safety/code-quality), each told to verify every claim against current code rather than trust this project's own docs. Security/auth and physical-access/data-privacy came back clean (session validation, RLS-as-auth anti-pattern absent, the 2026-09-05 wargaming session's Stripe-account-routing fix confirmed complete everywhere, IDOR checks consistent, RLS on health-data tables genuinely member-scoped). Payments/concurrency and chat-safety found real gaps, all fixed same session — full write-up in podhq-client's own `ROADMAP.md` ("Pre-launch review" section) since that's where all the code lives. This repo's own changes: two new migrations — `0091_create_booking_duplicate_slot_guard.sql` (closes a latent same-member/same-slot double-booking gap in `create_booking()`, unexploitable today since every real resource has `pod_capacity = 1` but live the moment one doesn't) and `0092_memberships_stripe_event_ordering.sql` (adds `memberships.last_stripe_event_created_at`, guarding the webhook's subscription-status update against Stripe's out-of-order delivery). Both applied live by Carl via Supabase's SQL Editor.

    **Worth carrying forward**: mid-review, Carl surfaced a real physical-safety fact that had never been documented anywhere — every pod has an in-pod panic button wired to a 24/7 monitoring company. This directly shaped the new physical-medical-emergency chat reply (leads with "press the panic button," 999 as backup) and belongs in podhq-client's reference material generally, not just this one reply's comment — flag for a future session if it comes up again without already being written down there.

63. **Rest-timer intelligence, session timer, and duration-feedback loops — 2026-09-07, same day.** Carl explicitly asked for a full written record of this stretch of work, distrustful of a chat-only summary — full detail lives in podhq-client's own `ROADMAP.md` ("Email branding, Android Play-prep fix, and the full rest-timer/session-timer/duration-feedback loop" section), since all the application code is there; this repo's change is migration `0093_workout_rest_intelligence.sql` (three columns: `workout_sets.rest_actual_seconds`, `workout_exercises.rest_change_reason`, `workout_sessions.started_at`/`duration_feedback`) plus a same-day comment correction (`d5b6741`) after the design changed mid-build.

    **Summary of what shipped**: the existing rest-timer UI (previously custom-workouts-only) now fires for every AI-generated exercise too, using the block-type rest baselines that already existed but were never actually applied; real rest taken feeds an intelligent per-exercise adjustment next time, explained in plain English. A real overall session timer, stamped server-side on first entering the workout *and* on a successful door unlock (tying it to when the member is genuinely in the pod) — the latter also auto-navigates both unlock surfaces straight into the workout screen with a one-time "Welcome, {name}" banner, skipping the AI intro screen. Self-reported "too long/too short/just right" feedback at session-complete.

    **A real mid-build correction, recorded honestly rather than smoothed over**: the duration-feedback mechanism was first built to adjust the AI-generated exercise *count* — Carl caught this as wrong before it shipped, since core compound lifts (bench press, rows, squats) must always stay and keep progressing regardless of session-length complaints. Reworked to adjust accessory (`isCompound: false`) exercises' own set count instead (2/3/4 sets), leaving every compound lift and the exercise-count logic completely untouched — verified explicitly in tests, including that a deload week's own reduced-volume mechanism takes priority over any duration feedback.

    **Explicitly discussed and scoped out, not built**: decline-detection (3 consecutive sessions of falling weight/reps on a key lift triggering a direct "is everything ok, do we need a deload" check-in) and locking accessory sets for 4 weeks once "just right" is confirmed. Both need real trend data — `getWorkoutHistory()` currently collapses each exercise's history to a single most-recent value, not a sequence — agreed to design this properly as its own next step rather than bolt it on ad hoc.

64. **Decline-detection built — 2026-09-08, picking up the approved-but-unbuilt plan from the previous session's stopping point.** Carl had gone to bed the night before right after approving a fully-designed plan (an `AskUserQuestion` round plus a Plan-agent-validated rule) without any code written; this session opened with "pick up where we left off," a recap of the approved rule (on request — "is there any algorythms or science for this decline dedection?"), then "yes go ahead and implement it." Built to the plan exactly, no scope or rule changes.

    **The rule, unchanged from design**: raw weight can't be compared across sessions since the rep target itself shifts across a training block's 4-week phase, so weight is normalized to **estimated one-rep-max** (Epley: `e1RM = weight_actual_kg * (1 + reps_actual / 30)`). A compound lift's e1RM across its **last 3 real appearances** (not calendar sessions — an A/B/C rotation means a lift doesn't run every session) must be non-increasing overall with at least one genuine decrease, AND the newest RPE isn't lower than the RPE from 2 appearances ago (missing RPE treated as "didn't ease off"). The RPE half is the standard autoregulation signal separating a member's own deliberate back-off (effort drops with output) from real overreaching (effort holds or rises while output falls).

    **All podhq-client code** (`coach-profile.ts`'s new `ExerciseAppearance`/`ExerciseTrend` types and `getWorkoutHistory()`'s new `trends` field; `types.ts`'s `DECLINE_CHECK_HISTORY_LOOKBACK_SESSIONS = 15`; new `decline-detection.ts`; `generateAndPersistSession`'s wiring; `workout-view.tsx`'s dismissible banner; `decline-detection.test.ts`, 12 cases) — full write-up in podhq-client's own `ROADMAP.md` ("Decline-detection — built and verified"), since that's where the actual code lives. This repo's only change is migration `0094_workout_decline_check.sql` — one nullable `workout_sessions.decline_alert_exercise_key` text column, no CHECK constraint, same convention as every migration since `0088`.

    **A real test-fixture bug caught before it shipped**, not after: one of the 12 test cases ("no compound in the plan is declining") first failed because the fixture's appearances were ordered wrong for a newest-first array — ascending weight by array position (60→62→64) actually encodes a *real decline* read backwards through time, not an improving trend. Caught by the test failing honestly rather than being written to pass; fixed by reversing to descending-by-array-position (64→62→60), which genuinely represents weight climbing session over session.

    **Verified**: `tsc --noEmit`, eslint, `npx vitest run` (217/217, up from 195), and `npm run build` all clean in podhq-client. **Not yet applied live** — migration `0094` needs Carl to run it via Supabase's SQL Editor, and the banner hasn't been confirmed against a real 3-appearance decline end-to-end (needs real logged sessions over time to occur naturally, not something fabricated to test — flagged as non-blocking, Carl to confirm whenever it naturally comes up).

65. **Decline-detection verified live end-to-end, real dev-test bug fixes, wger evaluated and rejected — 2026-09-08, same session.** Migration `0094` applied by Carl. Local dev server started for real browser verification (per this project's own "test in local dev first" convention) rather than trusting the code alone.

    **The old seeded dev test member (123) no longer existed** — swept up in the 2026-08-22 `members` table wipe and never recreated. Found the real current dev account (151, "Dev Test Member," Hove) by querying `members` for likely test rows. Checked its actual workout history first (`scripts/check-member-123-compound-history.sql`, retargeted) — it already had 2 real Barbell Bench Press appearances, so seeded the decline-test data against **Seated Row** instead (an exercise it had never done), avoiding any collision with real numbers. New disposable scripts committed alongside the demo-data ones already in the repo (`scripts/seed-decline-test-member-123.sql`, `remove-decline-test-member-123.sql`, `grant-test-credit-member-151.sql`, `find-test-member.sql`) — member 151 needed a credit grant (`manual_grant`, same convention as the 2026-09-05 wargaming session) to book a fresh test slot, since the decline check only ever runs at NEW session generation, never retroactively.

    **A real false alarm, chased down properly rather than assumed fixed by restarting things.** The banner didn't appear on the first several checks despite the API response confirming `declineAlertExerciseKey: "seated_row"` was correct every time (verified via direct `fetch()` calls from the page). Ruled out, in order: stale Next.js cache (killed the dev server, `rm -rf .next`, full restart — no change), a stale client bundle (grepped every loaded script for the banner's own text — genuinely absent at one point, present after a clean rebuild), and a hung mount effect (the page's own `generate()` on-mount fetch simply never fired on some loads, confirmed via `performance.getEntriesByType('resource')` showing zero API calls — root-caused to **Chrome throttling a backgrounded automation tab's JS**, not a real bug; a single click un-froze it). With a fully rendered, unthrottled page, a temporary render-time `console.log` confirmed `hasProgress: false`, `declineAlertExerciseKey: "seated_row"`, `declineAlertDismissed: false` — and the banner then rendered exactly as designed: *"Your Seated Row has dropped the last 3 times you did it — everything ok? Might be worth a deload week."* "Got it" dismissed it cleanly. All debug logging removed before committing; `git diff` confirmed the component file matched the already-committed version exactly.

    **Mid-verification, Carl caught something real**: picking Seated Row for the test surfaced that its catalog entry never encoded which he'd actually need — Hove's seated row station is dual-pulley specifically, not the single-pulley alternative `cable_machine` (the one generic equipment type, per Carl's own 2026-08-24 call) also covers. Renamed to "Dual-Pulley Seated Row" with an updated safety tip, matching the existing naming convention for other grip/equipment variants (`lat_pulldown_v_grip`, `lat_pulldown_double_handle`). Also surfaced, separately: neither Seated Row nor Standing Cable Row had a real filmed video, only the generic fallback demo photo — Carl filmed and uploaded a real one for **Standing Cable Row** mid-session (confirmed live via podhq-client's own read-only `/api/exercise-videos` endpoint, correctly keyed). Decision: keep Dual-Pulley Seated Row in the catalog flagged as still needing a video, since Standing Cable Row is now the better-supported option.

    **wger evaluated as a royalty-free fallback image/GIF source, real research not a guess, then rejected.** Checked wger's actual license terms (WebSearch, since a direct WebFetch hit their bot-protection page): the wger *software* is AGPL-3.0 (irrelevant, not being deployed), but its exercise *data* — images included — defaults to CC-BY-SA 3.0 **set per individual exercise**, meaning real attribution requirements and no guaranteed blanket license across the whole set; would have needed a manual per-exercise license check plus an attribution UI, not a quick wire-up. Presented honestly as a real but nontrivial feature rather than built ad hoc. Carl's call after seeing this ("Nah they are shit") was to drop it entirely — confirmed via follow-up he meant wger's own content quality, not just the licensing overhead.

    **Cleanup**: `remove-decline-test-member-123.sql` (despite its stale filename, correctly retargeted at member 151/Seated Row) removed the 3 seeded fake appearances, the one real session/booking the test generated (session 94, booking 180 — known exactly, not guessed, from the API responses during testing), and both credit ledger rows (the `manual_grant` +1 and its `booking_used` -1 spend) so the balance nets back to zero. Run live by Carl. Local dev server stopped and the test browser tab closed at session end.

    **Verified**: `tsc --noEmit`, eslint, full test suite (205/205, up from 195 at the start of the stretch), and a production build all clean throughout. **Not yet verified live**: the door-unlock → auto-navigate → welcome-banner flow end to end, and a same-session Android geolocation permission fix (found during Play Store submission prep) — neither has been watched happen on a real device yet.

66. **PDK holders linked automatically on first unlock — 2026-09-23.** Carl reported Steve (Fairford Leys owner) got "Your access isn't set up yet — contact staff" at the door: podhq-client's unlock route requires `members.pdk_holder_id`, which only Carl's own test account (member 157, via `0100`) had ever had set, by hand — so every other PDK-gym member was locked out. Carl's call: "I shouldn't have to do that." New `src/lib/pdk.ts` (token helper, `ensureAccessHolder`, `addToAccessGroup`, `virtualRead`, endpoints checked against developer.pdk.io/web/2.0 holders/groups docs); `/api/pdk/unlock` now accepts `newHolder` (name/email) instead of `holderId`, reuses an existing PDK holder with an exact email match or creates one, adds it to the "Booking Access" group, unlocks, and returns `holderId` (even on failure) for podhq-client to save. A failed virtual-read re-adds to the group (GymFlow manages that group too) and retries once after 2s. First design always created a fresh holder to avoid granting standing access to a fob/PIN-carrying GymFlow holder — Carl confirmed holders here carry no fob/PIN, so email matching is safe. Also traced a Brighton "AAL2 session is required" password-reset failure to the shared Hove staff login (2FA via podHQ) being entered in the member app — not a bug, no change. **Verified live 2026-09-23 14:49**: a fresh Fairford Leys test member's first unlock created their PDK holder, added it to Booking Access, and PDK's audit log shows "Access allowed" on "Entrance" — no manual step, no retry needed. Client-side follow-up the same session (podhq-client `fef9227`): the Unlock buttons no longer hard-stop without GPS; the server decides. Same-day speed follow-up after Carl found the unlock slow: `getSystemToken` now caches the PDK system token per system for 5 minutes (dropped on a 401, and the route's retry re-fetches), removing two token round trips from every unlock.

67. **Staff "Delete member" + emergency-contact migration committed — 2026-09-23.** Work written in an earlier session and left uncommitted; `0096` (members.emergency_contact_name/phone, used by podhq-client's Profile card) and `0097` (`delete_member_cascade(p_member_id)`, every hand-discovered FK dependant in one function) were already applied live by Carl. `/pods/members/[id]` gets a two-step "Delete member" (confirm → `/api/pods/members/[id]/delete-account`), gym-scoped via `getGymScope`/`resolveGym`, runs the cascade then deletes the `auth.users` row via the Admin API, audit-logged as `staff_member_deleted`. The `0096` file on disk had a corrupted last line (`tehe` for `text;`) — fixed in the file only; the live columns were already correct. **Known gap**: deleting a member doesn't remove their PDK holder (auto-created since #66) — flagged to Carl for GDPR erasure, not built.


68. **Door Traffic — Kisi + PDK entry history cached monthly, `/door-traffic` page — 2026-09-25.** Carl asked for last month's door data now PDK is integrated, then for the same from Kisi, then (his call) to cache it month by month rather than rely on vendor retention, and a podHQ page to see it. Both vendors keep long history: PDK's reports API (`/reports/events/generate`, async, 204 then poll `/requests`) had Fairford Leys from its install (nothing in May 2026); Kisi's `event_sets` API (max 90-day window per request, not 90-day retention) returned Aylesbury events from before Jan 2024. **Counting rules, verified against Aug-Sep 2026 at every site**: Kisi entry = `lock.unlock` by a User (actor-less unlocks are the exit button, 1:1 with `controller_input.triggered`; reader taps log their own user unlock so `reader.access` isn't counted); PDK entry = `cardread` with result `allowed`/`multiallowed`, denied = `denied`. New: `0102` (`door_entries`, one row per entry/denied attempt with holder name + email — Carl confirmed storing both, for matching to members; `kisi_place_gyms`, six Kisi places incl. "My Fit Pod: Thomley" = Oxford East), `src/lib/door-history.ts` + `/api/door-history/sync` (CRON_SECRET, monthly cron `0 4 1 * *`, `?month=YYYY-MM` to re-run/backfill, idempotent on `(door_system, source_event_id)`, 500 on any site failing), `0103` `door_entry_summary()` (all aggregation in SQL — table is six figures of rows; UK-time buckets; person = lowercased email, visits = distinct days; EXECUTE revoked from anon/authenticated), `/door-traffic` page (period presets, admin gym picker, tiles, entries/denied chart, weekday×hour heatmap, by-gym table, most frequent visitors). Needs `KISI_API_KEY` in podHQ's Vercel env (Carl says already there). Aug 2026 synced rows matched the ad-hoc pull exactly at all 7 sites; backfilled locally to each site's install date (Aylesbury Jan 2023, Berkhamsted Jan 2024, Milton Keynes Oct 2024, Basingstoke Jan 2025, Oxford East Jul 2025, Kingston Oct 2025, Fairford Leys Jun 2026). Wellhub check-in enforcement plan (uses this data to size unpaid visits) is in podhq-client's ROADMAP.md. **Security fix found along the way**: `delete_member_cascade` (`0097`) was `security definer` with Supabase's default EXECUTE grant to anon — anyone with the anon key could delete any member via `/rest/v1/rpc`. `0104` revokes it; verified the anon key now gets 401 on both functions. **Not done**: turning `door_entries` into `attendance` rows (step 2 — run alongside the GymFlow CSV for a month or two and compare first); Brighton (PDK system not mapped); Crewe/Hackney/Hove have no door data here.
