...
60. **Exercise video library filled (75 videos) + catalog extended — 2026-09-06.** Carl filmed his own technique videos for the `/exercise-videos` admin page (`0085`, built 2026-09-04) and wanted them all uploaded and matched, one folder of ~75 raw `.mp4` files (`C:\Users\carls\OneDrive\Desktop\App Videos`) with no reliable naming convention.
    ...
    **`pull_up_bar` added as a new equipment type**, duplicated correctly across both repos' `EQUIPMENT_TYPES` (`podhq-client/src/lib/coach/types.ts`, `podHq/src/lib/data/types.ts`) plus `EQUIPMENT_LABELS` and the `/pods` calendar settings checkbox list (`calendar-view.tsx`) — Carl confirmed Hove actually has one before this was added, gating `hanging_knee_raise`/`hanging_leg_raise`. **Follow-up still open**: the equipment type existing in code doesn't mean Hove's `pod_resources.equipment` row actually has it checked yet — needs confirming in `/pods` before those two exercises can ever actually get prescribed.
