...
import { deleteOutgoing } from "@/lib/data/outgoings";
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rateLimit = await checkRateLimit(user.id, "/api/outgoings/[id]");
  ...
  try {
    ...
    const deleted = await deleteOutgoing(id, gym);
    if (!deleted) {
    ...
    console.error("[api/outgoings/[id]]", { userId: user.id, error: err instanceof Error ? err.message : err });
    return NextResponse.json(
