...
export default function CustomerDirectoryError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[members/directory]", error.digest ?? error.message);
  }, [error]);

  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 px-4 text-center">
      <p className="text-sm font-semibold tracking-wide text-accent">PodHQ</p>
      <h1 className="text-lg font-semibold text-foreground">Couldn&apos;t load the customer directory</h1>
