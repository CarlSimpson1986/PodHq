...
export default function DoorTrafficError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("[door-traffic]", error.digest ?? error.message);
  }, [error]);

  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 px-4 text-center">
      <p className="text-sm font-semibold tracking-wide text-accent">PodHQ</p>
      <h1 className="text-lg font-semibold text-foreground">Couldn&apos;t load door traffic</h1>
