...
21. **Staff sell/comp packs & memberships (`/pods/members/[id]`), card-on-file, and a live per-gym pricing catalog (`/setup`)** — one long session, 2026-08-15, that grew substantially from its original scope after live comparison against GymFlow's own equivalent screens.
    ...
    **Full pivot to a live, per-gym pricing catalog (Setup)**, after the user compared against GymFlow's own Setup → Memberships/Credits pages and asked for the same "create new things" capability rather than fixed arrays. Went through two real design corrections before landing: first built as admin-managed and global (one catalog for every gym) under `/admin/catalog`; the user then clarified "admin" specifically means the franchisor and pricing isn't a franchisor-level decision, so it moved to a **standalone "Setup" nav item, owner-only, per-gym** — a real, not cosmetic, distinction in this app (matches "admin is only for me the franchisor," not a general elevated-staff role).
    ...
    - `AppShell` now has both an admin-only and an owner-only href list (`/admin` vs `/setup`) — the inverse case of the existing pattern, since this is the first owner-only (not admin-only) page in the app.
    - **Verified live** (as admin, the only role available for scripted testing — MFA blocks scripting an owner login, same known limitation as Stages 15/19): catalog CRUD confirmed end-to-end (created via `/setup`... no — confirmed via direct edit/disable/enable on the seeded rows, price edit reflected immediately in the sell panel with zero restart), `/setup` correctly returns "Owners only." for an admin session, and "Setup" correctly does not appear in admin's sidebar while "Admin" does. **Not yet verified: the owner-side UI itself** (rendering, create/edit forms, gym-scoping) — needs a real owner login before relying on it.
