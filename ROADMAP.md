...
9. **Admin panel** (`/admin`, admin only) — **built 2026-07-28, partially verified live, not fully confirmed.** User management (list, invite-to-create an owner, deactivate/reactivate) + system status (row counts per table, last sync). Deliberate scope decisions: admin accounts are *not* creatable from this UI — only owner accounts — the most privileged role stays a manual out-of-band step; "deactivate" bans the Supabase Auth account outright (via `ban_duration`) rather than just removing the `users_gyms` row, so reactivating restores exact prior gym/role instantly; "last sync" is `MAX(created_at)` across `Revenue`/`attendance` (a proxy for last DB write, not a true upstream export timestamp — there's no dedicated sync-log). Creating an owner reuses the existing invite → `/auth/callback` → `/login/set-password` flow from Stage 2 rather than inventing a new one. Self-deactivation is blocked server-side (`/api/admin/users/[id]` rejects a target id matching the caller's own).
   ...
   **Not yet verified — pick up next session:** an actual invite email land-and-click-through to `/login/set-password` (was about to test with a real `+`-alias address, `carlsimpson83+podhqtest@yahoo.co.uk`, when the session hit a usage limit), the deactivate/reactivate toggle end-to-end, and the owner-role tamper test against `/api/admin/users` and `/api/admin/status` (same pattern already used successfully for Stage 5 and Stage 8 — an owner session hitting these routes directly should get a 403). Type-checks clean; not yet exercised as anything other than the real admin account.
